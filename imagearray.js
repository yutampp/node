...
fs.readFile("imagelist.out","utf8").then(data=>{
  ...
  const promise = ()=>{
    if(urls.length==0){
      ...
      const ciid = url.split("&")[2].replace("ciid=","");
      options.url = url;
      options.encoding = null;
      return request(options).then(data=>{
        const image = new Buffer.from(data);
        fs.writeFile(`images/${cid}-${ciid}.png`,image);
        result[cid] = result[cid] || [];
        result[cid].push(`${cid}-${ciid}.png`);
        return new Promise(resolve=>setTimeout(()=>promise().then(data=>resolve(data) ),1000) )
      })
    }
  };
  return promise()
}).then(data=>{
  return fs.writeFile("imagearray.out",JSON.stringify(data) )
})
